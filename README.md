...
```bash
gcc -Wall -Wextra -g -O3  main.c lib/import.c lib/convolution.c lib/pooling.c lib/dense.c lib/output.c lib/backprop.c -o cnn
```
...
|----------------------------------------|
```
When trained for 5 epochs, we get the following results with a learning rate of 0.001:
```bash
|----------------------------------------|
| Average Loss: 0.309487 | Accuracy: 90% |
|----------------------------------------|
````